...
psql -c "INSERT INTO sales VALUES (generate_series(1001,2000),(SELECT max(empno) FROM emp),md5(random()), now(), (random()*100)::int);"
...
# Randomize sale dates and set the customer name for each row based on the empno/salesperson
MAXID=`psql -Atc "SELECT max(id) FROM sales"`
for id in `seq 1 ${MAXID}`
  do
  psql -qc "UPDATE sales SET sale_date = (select current_date - random() * interval '1 year') WHERE id = ${id}"
