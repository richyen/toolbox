...
echo 'Step 0. -- Set up Streaming Replication'
...
echo 'Step 9. Make old primary a slave'
sed -i "s/#hot_standby.*/hot_standby=on/" /tmp/5555/data/postgresql.conf
sed -i "s/^hot_standby.*/hot_standby=on/" /tmp/5555/data/postgresql.conf

echo 'Step 10. Create a recovery.conf file in the old master cluster'
...
echo 'Step 14. -- switchback -- Now stop new primary'
pg_ctl -D /tmp/6666/data stop

echo 'Step 15. -- switchback -- Check fot sync status of new Standby [5555] before promoting'
psql -p 5555 edb -c "select * from pg_stat_replication;"

echo 'Step 16. -- switchback -- do pg_ctl promote'
pg_ctl -D /tmp/5555/data promote

echo 'Step 17. -- switchback -- Restart stopped new primary [6666] as new standby'
...
echo 'Step 18. -- switchback -- verify replication is working'
sleep 5
psql -p6666 -c "Select pg_is_in_recovery;"

psql -p5555 -c "create table new_master(id numeric);"
psql -p5555 -c "insert into new_master values (generate_series(1,1234))"
...
psql -p6666 -c "select count(*) from new_master"
psql -p7777 -c "select count(*) from new_master"
