...
# Demo script to set up RADIUS authentication on EPAS 9.x on Centos 6
...
sed -i "s/host   all.*trust/host   all         all      0.0.0.0\/0  radius  radiusserver=127.0.0.1 radiussecret=testing123 radiusport=1812/" /var/lib/ppas/9.5/data/pg_hba.conf
psql -c "select pg_reload_conf()"
 
# Log in with wrong RADIUS password
set -x
PGPASSWORD='foobar' psql -d edb -U steve -h 127.0.0.1
# Log in with correct RADIUS password
PGPASSWORD='testing' psql -d edb -U steve -h 127.0.0.1
